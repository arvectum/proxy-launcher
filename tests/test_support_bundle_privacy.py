...
class SupportBundlePrivacyTests(unittest.TestCase):
    ...
    def test_bundle_redacts_secret_shapes_in_structured_and_plaintext_logs(self):
        # Secret-shaped fixtures are assembled only at runtime so repository-wide
        # secret scanning remains meaningful and the Git history contains no
        # token/private-key lookalikes.
        provider_token = "gh" + "p_" + ("A" * 26) + "123456"
        jwt_token = ".".join((
            "eyJhbGciOiJIUzI1NiJ9",
            "eyJzdWIiOiJhcGwtaGlkZGVuLXVzZXIifQ",
            "signature" + "1234567890",
        ))
        pem_begin = "-" * 5 + "BEGIN " + "PRIVATE KEY" + "-" * 5
        pem_end = "-" * 5 + "END " + "PRIVATE KEY" + "-" * 5
        canaries = (
        ...
        private_key = "\n".join((pem_begin, canaries[5], pem_end))
        log_text = "\n".join((
            json.dumps({
