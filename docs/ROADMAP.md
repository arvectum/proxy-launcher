...
- **IN PROGRESS — [Win] P0.2** — preflight PASS: a fresh recovery checkout was obtained from GitVerse at exact commit `678efda6df68c93db8474c810abd73bca72735b2`; the governed P0.1 archive was staged at `30,996,168` bytes / SHA-256 `4a55f101bdd15a956c9bc4249fdbb694abadd682a3340c0f5ef08c174880a886` and passed the canonical verifier again. Exact Inno Setup `6.7.1` is not currently pre-staged, so installer recovery remains blocked. Windows Sandbox is unavailable and no disposable VM exists yet. The apparent hardware-virtualization blocker has been resolved diagnostically: after BIOS enablement Windows reports `HypervisorPresent = YES` and VBS/VSM status code `2` (**RUNNING**), so the negative processor-WMI virtualization/SLAT flags are not treated as proof of disabled firmware virtualization. Diagnosis: `WINDOWS_VBS_HYPERVISOR_ACTIVE`; BIOS virtualization gate **PRESUMED PASS**; VirtualBox next step **SAFE_TO_PROVISION** without disabling security controls. Evidence: `docs/evidence/P0_2_VIRTUALIZATION_DIAGNOSTIC_RESOLUTION.json`. Next boundary: verify/install base Oracle VirtualBox, stage verified Windows 11 x64 installation media, create `ARVECTUM-P0-2-RECOVERY`, create clean snapshot, prove hypervisor-level network disconnect, then rerun the endpoint-denied portable recovery proof.
...
1. **[Win] P0.2 VirtualBox recovery-environment provisioning:** acquire and hash-verify the supported Oracle VirtualBox Windows-host installer, install the base package without Extension Pack, stage verified Windows 11 x64 installation media, create/resettable `ARVECTUM-P0-2-RECOVERY`, create `P0-2-CLEAN-BASELINE`, and prove hypervisor-level network disconnect. Do not disable VBS/security controls merely for performance. After provisioning, rerun the endpoint-denied portable recovery proof. Exact Inno Setup 6.7.1 must separately be brought under controlled/pre-staged storage before installer recovery can close P0.2.
