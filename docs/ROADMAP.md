...
1. **[Win] P0.1 final closure:** physically unplug `ARVECTUM-1` and store it separately; then attach/retrieve the exact offline bytes on Windows, run the canonical verifier with current repository locks and package-index access disabled, and write final `P0_1_COMPLETION_EVIDENCE.json`.
