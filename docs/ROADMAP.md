...
- **IN PROGRESS — [Win] P0.2** — preflight PASS: a fresh recovery checkout was obtained from GitVerse at exact commit `678efda6df68c93db8474c810abd73bca72735b2`; the governed P0.1 archive was staged at `30,996,168` bytes / SHA-256 `4a55f101bdd15a956c9bc4249fdbb694abadd682a3340c0f5ef08c174880a886` and passed the canonical verifier again. Exact Inno Setup `6.7.1` is not currently pre-staged, so installer recovery remains blocked. Windows Sandbox is unavailable, but the VirtualBox recovery path is now proven through the VM-engine boundary: Oracle VirtualBox `7.2.14` revision `174565` was hash-verified against Oracle SHA256SUMS and Authenticode-verified, installed without Extension Pack, and `VBoxManage` is operational. Dedicated VM `ARVECTUM-P0-2-RECOVERY` exists with 4096 MB RAM, 2 vCPU, EFI and `nic1=none`; the x64 VM engine starts successfully, running state was observed, final poweroff passed, VBS/security controls were not deliberately disabled, and hypervisor-level network disconnect capability is proven. Evidence: `docs/evidence/P0_2_VIRTUALBOX_PROVISIONING_EVIDENCE.json`. Recovery environment is still **NOT READY** because no Windows guest or clean snapshot exists. Next boundary: stage a verified Windows 11 x64 ISO, install the clean guest with NIC disabled, create `P0-2-CLEAN-BASELINE`, then stage the frozen GitVerse source + P0.1 archive and run the endpoint-denied portable recovery proof.
...
1. **[Win] P0.2 Windows guest clean-baseline provisioning:** stage and hash-verify a Windows 11 x64 installation ISO, attach it to existing `ARVECTUM-P0-2-RECOVERY` with `nic1=none`, install the clean x64 guest, and create/verify snapshot `P0-2-CLEAN-BASELINE` before any product inputs are introduced. Then stage the frozen GitVerse source and P0.1 controlled archive using local/offline transport and rerun the endpoint-denied portable recovery proof. Exact Inno Setup 6.7.1 must separately be brought under controlled/pre-staged storage before installer recovery can close P0.2.
