...
- **IN PROGRESS — [Win] P0.2** — preflight PASS, clean recovery baseline PASS and controlled-input staging PASS. Recovery source authority remains frozen at exact GitVerse commit `678efda6df68c93db8474c810abd73bca72735b2`; governed P0.1 archive identity remains `30,996,168` bytes / SHA-256 `4a55f101bdd15a956c9bc4249fdbb694abadd682a3340c0f5ef08c174880a886`. The verified snapshot `P0-2-CLEAN-BASELINE` UUID `e5abd145-780c-457c-8b8c-a4aa01581716` was restored before staging. With VirtualBox NIC `NONE`, a host-local VDI was attached at Port 1 and exposed as `D:\P0_2_INPUTS`; frozen source was staged at `D:\P0_2_INPUTS\source` and the governed archive at `D:\P0_2_INPUTS\controlled-inputs`. Source identity was verified host-side by `git rev-parse` plus `source_manifest.json`; archive identity matched and the operator reports canonical verifier PASS for the host-side prepared VDI. This record does not claim a separate in-guest verifier execution. CPython remains uninstalled, pip unused, and product/portable/installer recovery builds have **not** started. Evidence: `docs/evidence/P0_2_CONTROLLED_INPUT_STAGING_EVIDENCE.json`. Next boundary: run the genuine endpoint-denied portable recovery proof inside the guest, beginning with an in-guest canonical archive verification, installing CPython `3.12.10` only from P0.1 and using only the exact eight-wheel hash-locked wheelhouse with `PIP_NO_INDEX=1` / `offline-hash-locked` mode. Exact Inno Setup `6.7.1` remains separately blocked as `BLOCKED_MISSING_PRESTAGED_INNO_6_7_1`, so P0.2 remains open even if portable recovery passes.
...
1. **[Win] P0.2 endpoint-denied portable recovery proof:** keep VirtualBox NIC `NONE`; use only staged frozen GitVerse source `678efda6df68c93db8474c810abd73bca72735b2` and the exact governed P0.1 archive; run the canonical archive verifier in the guest, install CPython `3.12.10` only from controlled inputs, enforce `PIP_NO_INDEX=1` / `offline-hash-locked` mode with the exact eight-wheel wheelhouse, run tests/package-contract/branding/SBOM/locked-dependency checks, verify endpoint denial before and after build, and export evidence without public acquisition. Exact Inno Setup `6.7.1` must separately be brought under controlled/pre-staged storage before installer recovery can close P0.2.
