...
- **IN PROGRESS — [Win] P0.2** — preflight PASS: a fresh recovery checkout was obtained from GitVerse at exact commit `678efda6df68c93db8474c810abd73bca72735b2`; the governed P0.1 archive was staged at `30,996,168` bytes / SHA-256 `4a55f101bdd15a956c9bc4249fdbb694abadd682a3340c0f5ef08c174880a886` and passed the canonical verifier again. Exact Inno Setup `6.7.1` is not currently pre-staged, so installer recovery remains blocked. The attempted portable-recovery sub-gate did not start because neither Windows Sandbox nor a pre-existing clean/disposable Windows x64 VM was available on the execution host; blocker `NO_DISPOSABLE_WINDOWS_RECOVERY_ENVIRONMENT` is recorded in `docs/evidence/P0_2_DISPOSABLE_ENVIRONMENT_BLOCKER.json`. Next required boundary: provision a clean/disposable Windows x64 recovery VM/environment without weakening the endpoint-denial acceptance gate.
...
1. **[Win] P0.2 recovery-environment provisioning:** provision a clean/disposable Windows x64 VM/environment suitable for the exact GitVerse `678efda6...` recovery source and P0.1 controlled archive. Do not relax the requirement for public endpoints to remain denied during the governed build phase. After provisioning, rerun the endpoint-denied portable recovery proof. Exact Inno Setup 6.7.1 must separately be brought under controlled/pre-staged storage before installer recovery can close P0.2.
