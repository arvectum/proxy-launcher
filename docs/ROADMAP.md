...
- **IN PROGRESS — [Win] P0.2** — preflight PASS: a fresh recovery checkout was obtained from GitVerse at exact commit `678efda6df68c93db8474c810abd73bca72735b2`; the governed P0.1 archive was staged at `30,996,168` bytes / SHA-256 `4a55f101bdd15a956c9bc4249fdbb694abadd682a3340c0f5ef08c174880a886` and passed the canonical verifier again. Exact Inno Setup `6.7.1` is not currently pre-staged, so installer recovery remains blocked. The portable-recovery run has not started because the current host has no disposable Windows environment. Provisioning audit additionally reported hardware virtualization **NO**, SLAT **NO**, Hyper-V unavailable on the reported Home edition, no third-party hypervisor, and no Windows ISO. Evidence: `docs/evidence/P0_2_HARDWARE_VIRTUALIZATION_BLOCKER.json`. Next required human boundary: enable/prove VT-x or AMD-V/SVM in BIOS/UEFI, reboot, re-verify virtualization, then provision a clean/resettable Windows x64 VM with hypervisor-level network disconnect capability. If the hardware cannot expose virtualization, use another controlled x64 host rather than weakening P0.2 acceptance.
...
1. **[Win] P0.2 hardware-virtualization human gate:** enable VT-x / AMD-V / SVM in BIOS/UEFI on the intended Windows host, reboot, and verify virtualization is actually exposed to Windows. If it cannot be exposed, move the recovery drill to another controlled x64 host. Only after this gate passes, provision a clean/resettable Windows x64 VM and rerun the endpoint-denied portable recovery proof. Exact Inno Setup 6.7.1 must separately be brought under controlled/pre-staged storage before installer recovery can close P0.2.
